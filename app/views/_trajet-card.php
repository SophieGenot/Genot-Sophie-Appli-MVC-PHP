...
<div class="trajet-card">
    ...
    <div class="mt-3 d-flex gap-2">
        ...
        <?php if ($mode === 'mes_trajets'): ?>
            ...
            <a href="index.php?action=edit_trajet&id=<?= htmlspecialchars($id_affichage); ?>" class="btn btn-primary">Modifier</a>
            
            <form action="index.php?action=delete_trajet" method="POST" class="m-0" onsubmit="return confirm('Supprimer votre trajet ?');">
        ...
        <?php elseif ($mode === 'admin'): ?>
            <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#trajetModal<?= htmlspecialchars($id_affichage); ?>">Infos</button>
            
            <form action="index.php?action=delete_trajet" method="POST" class="m-0" onsubmit="return confirm('Supprimer ce trajet (Action Admin) ?');">
                <input type="hidden" name="trajet_id" value="<?= htmlspecialchars($id_affichage); ?>">
