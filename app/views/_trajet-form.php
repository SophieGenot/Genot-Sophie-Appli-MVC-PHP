...
<main class="container mt-4">
    ...
    <form action="index.php?action=<?= isset($trajet) ? 'edit_trajet&id=' . htmlspecialchars($trajet['id']) : 'create_trajet'; ?>" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="agence_depart_id" class="form-select" required>
                ...
                <?php foreach($agences as $agence): ?>
                    <option value="<?= htmlspecialchars($agence['id']); ?>" <?= isset($trajet) && $trajet['agence_depart_id'] == $agence['id'] ? 'selected' : ''; ?>>
        ...
        <div class="mb-3">
            ...
            <select name="agence_arrivee_id" class="form-select" required>
                <option value="">-- Choisir --</option>
                <?php foreach($agences as $agence): ?>
                    <option value="<?= htmlspecialchars($agence['id']); ?>" <?= isset($trajet) && $trajet['agence_arrivee_id'] == $agence['id'] ? 'selected' : ''; ?>>
                        <?= htmlspecialchars($agence['nom']); ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="gdh_depart" class="form-control" required
                   value="<?= isset($trajet) ? htmlspecialchars(date('Y-m-d\TH:i', strtotime($trajet['gdh_depart']))) : ''; ?>">
        </div>
        <div class="mb-3">
            ...
            <input type="datetime-local" name="gdh_arrivee" class="form-control" required
                   value="<?= isset($trajet) ? htmlspecialchars(date('Y-m-d\TH:i', strtotime($trajet['gdh_arrivee']))) : ''; ?>">
        </div>
        <div class="mb-3">
